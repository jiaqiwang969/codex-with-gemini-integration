...
impl OutgoingMessageSender {
    ...
    pub fn send_response(&self, id: serde_json::Value, result: serde_json::Value) {
        // Convert serde_json::Value id back to RequestId
        ...
        let response = JSONRPCResponse {
            ...
            id: request_id,
            result, // Result is just an alias for serde_json::Value
        };
...
impl MessageProcessor {
    pub fn new(sender: OutgoingMessageSender) -> Self {
        ...
        let secret_id = env::var("TENCENTCLOUD_SECRET_ID").ok();
        let secret_key = env::var("TENCENTCLOUD_SECRET_KEY").ok();

        if secret_id.is_none() || secret_key.is_none() {
            error!("Warning: TENCENTCLOUD_SECRET_ID and TENCENTCLOUD_SECRET_KEY not set");
        }
    ...
    async fn handle_initialize(&mut self, id: serde_json::Value, params: serde_json::Value) {
        ...
        let result = InitializeResult {
            ...
            server_info: Implementation {
                name: "hunyuan-mcp-server".to_string(),
                title: None,
                version: "0.1.0".to_string(),
                user_agent: None,
            },
            instructions: Some("Hunyuan AI3D MCP Server - Generate 3D models from text and images using Tencent Cloud's Hunyuan AI3D API.".to_string()),
        };
        ...
        if self.secret_id.is_none() || self.secret_key.is_none() {
            let result = CallToolResult {
                content: vec![ContentBlock::TextContent(TextContent {
                    ...
                    text: "Error: Tencent Cloud credentials not configured. Please set TENCENTCLOUD_SECRET_ID and TENCENTCLOUD_SECRET_KEY environment variables.".to_string(),
                    annotations: None,
                })],
                is_error: Some(true),
