    ...
    async fn handle_event(&mut self, tui: &mut tui::Tui, event: AppEvent) -> Result<bool> {
        ...
        match event {
            ...
            AppEvent::UpdateModel(model) => {
                ...
                // Keep the in‑memory provider configuration aligned with the selected model
                // family. This mirrors the logic used inside `SessionConfiguration::apply`
                // so that the TUI's status output and session logs reflect the effective
                // provider.
    ...
    async fn activate_delegate_session(
        ...
        let state = if let Some(state) = self.delegate_sessions.remove(&conversation_id) {
            ...
            let model_family = models_manager
                .construct_model_family(&session.config.model, &session.config)
                .await;
