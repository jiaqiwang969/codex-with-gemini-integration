...
当前 Codex 集成的 Gemini 3 Pro Preview 模型如下：

\begin{itemize}
  \item \texttt{gemini-3-pro-preview-codex}：使用 Germini 风格 Prompt，并配合 Codex 工具调用行为；
  \item \texttt{gemini-3-pro-image-preview}：图像版，支持图像理解与图像生成（返回 \verb|inlineData| 图片数据）。
\end{itemize}

在请求的 \verb|generationConfig| 中，我们用 \verb|thinkingConfig| 来区分
Gemini 3 与 Gemini 2.x 的 thinking 行为，同时遵循 Gemini
“\verb|thinkingLevel| 与 \verb|thinkingBudget| 只能二选一”的校验：

\begin{itemize}
  \item Gemini 3：固定使用 \verb|thinkingLevel = "high"|，并设置 \verb|includeThoughts = true|，不再显式设置 \verb|thinkingBudget|；
  \item Gemini 2.x：设置 \verb|thinkingBudget = 8192|，不给 \verb|thinkingLevel|，必要时（高 reasoning effort）才附带 \verb|includeThoughts = true|；
  \item 图像模型（例如 \verb|gemini-3-pro-image-preview|）：直接省略 \verb|thinkingConfig|，避免图片模型在看到 level/budget 组合时返回 400。
\end{itemize}
...
\begin{itemize}
  ...
  对于以 \verb|/v1| 结尾的地址，Codex 会自动切换到 \verb|/v1beta|。
  \item \textbf{Key 读取顺序}：
...
\begin{enumerate}[label=\arabic*)]
  ...
  \item （可选）配置 \verb|toolConfig| 控制函数调用行为：
  \begin{itemize}
    \item 默认 \verb|mode = "auto"|，由模型自行决定是否调用工具；
    \item （默认启用）对“分析/读仓库”类任务的首轮请求，可设置 \verb|mode = "any"| 并提供
    \verb|allowedFunctionNames|（例如 \verb|read_file|/\verb|grep_files|/\verb|list_dir|），
    强制模型先读取上下文，避免过早直接回答；当下一轮已经有 functionResponse 后再回到 \verb|auto|；
    \item 可通过环境变量 \verb|CODEX_GEMINI_FORCE_READ_TOOLS_FIRST_TURN| 强制关闭该策略：
    \verb|0| 强制关闭（默认开启；\verb|1| 也表示开启）。
  \end{itemize}
  ...
  \begin{itemize}
    ...
    \item \verb|thinkingConfig| 如前所述（Gemini 3 使用 \verb|thinkingLevel=high|，Gemini 2.x 使用 \verb|thinkingBudget=8192|）。
  \end{itemize}
...
\begin{itemize}
  ...
  \item 用 \verb|call_id| 作为键，把 \verb|name| 和 \verb|thoughtSignature| 保存到 map，
  供之后的 functionResponse 逐条配对（支持同一轮多个工具调用）。
\end{itemize}
...
ResponseItem::FunctionCallOutput { call_id, output } => {
    ...
    let (output_text, mut inline_parts) = build_gemini_function_response_payload(output);
    let response_value = serde_json::json!({
        "output": output_text,
        "success": output.success.unwrap_or(true)
    });
    let part_thought_signature = thought_signature.clone();
    let nested_parts = if is_gemini_3 && !inline_parts.is_empty() {
...
同时，Gemini 3 会把 \verb|inlineData| 内嵌到 \verb|functionResponse.parts|，
而 Gemini 2.x 则把它们作为 siblings 发送。
...
\begin{enumerate}[label=\arabic*)]
  ...
  \item 调整 \verb|thinkingConfig| 构造：Gemini 3 固定发送 \verb|thinkingLevel|，Gemini 2.x 使用 \verb|thinkingBudget|，图片模型省略该字段以满足 level/budget 互斥校验；
  \item 实现 thoughtSignature 的完整传递：解析、存储、回放，并在缺失时兜底；
  \item 增加针对 Gemini 的工具调用 loop 检测，阈值从 5 提升到 100；
  \item 为“分析/读仓库”类任务增加首轮 \verb|toolConfig| 策略（\verb|any| + \verb|allowedFunctionNames|），并为 Gemini 家族默认开放 \verb|read_file|/\verb|grep_files|/\verb|list_dir|；
