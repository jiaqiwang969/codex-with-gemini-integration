...
低思考和高思考模型，同时遵循 Gemini “\verb|thinkingLevel| 与
\verb|thinkingBudget| 只能二选一”的校验：

\begin{itemize}
  \item thinking 模型：\verb|thinkingLevel = "high"|，且 \verb|includeThoughts = true|，不再显式设置 \verb|thinkingBudget|；
  \item 非 thinking 文本模型：仅设置 \verb|thinkingBudget = 32768|，不给 \verb|thinkingLevel|，让长链路工具调用有足够预算的同时避免冲突；
...
\begin{enumerate}[label=\arabic*)]
  ...
  \item 调整 \verb|thinkingConfig| 构造：thinking 模型发送 \verb|thinkingLevel|，非 thinking 文本模型发送较大的 \verb|thinkingBudget|，图片模型省略该字段以满足 level/budget 互斥校验；
